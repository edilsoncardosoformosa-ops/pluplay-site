...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById('confirmPassBtn').addEventListener('click', async () => {
        ...
        if (res.success) {
            alert('Acesso liberado via senha!');
    ...
    // Inicializa notas de usuário
    renderUserAllowedNote();
});

// Garantir que quizData seja um objeto com tópicos
if (!window.quizData) window.quizData = {};
if (Array.isArray(window.quizData)) {
    window.quizData = { "Lport": window.quizData };
}

// Renderiza nota de usuário
